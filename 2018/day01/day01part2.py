...
file = open("2018/day01/input.txt")
...
for indexTarget in range(len(frequencyList)):
    for indexSource in range(len(frequencyList)):
        ...
        if deltaElements % shift == 0 and (deltaElements / shift > 0):
            freqRepetitions.append(
                [indexSource, int(deltaElements / shift), indexTarget]
            )
...
if len(freqRepetitions) == 0:
    ...
    print(
        ...
        + str(frequencyList[firstRepetition[0]] + firstRepetition[1] * shift)
    )
